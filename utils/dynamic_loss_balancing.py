...
def default_fixed_coefficients(list_coef=None,
                               list_loss=None):
    """
    Returns default ponderated loss with fixed parameters.
    """
    if len(list_coef) != len(list_loss):
        raise ValueError("The length of list_params must be equal to the length of list_losses.")
    
    return sum(param * loss for param, loss in zip(list_coef, list_loss))
